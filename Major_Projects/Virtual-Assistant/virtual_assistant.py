...
def respond(voice_data):
    ...
    if there_exists(["what is your name", "what's your name", "tell me your name"]):
        if person_obj.name:
            ...
            engine_speak("i am your alexa. what's your name?")
    ...
    if there_exists(["pod", "tell me about pod"]):
        engine_speak(
            "Pods are the smallest, most basic deployable objects in Kubernetes. A Pod represents a single instance of a running process in your cluster. Pods contain one or more containers, such as Docker containers.")
    if there_exists("what inside pod defination"):
        engine_speak("apiversion,kind,metadata,spec,status")
    if there_exists(""):
        engine_speak("Container,templates,initcontainers,dnspolicy,volumes")
    if there_exists(["your name should be","from now your name shoule be"]):
    ...
    # 3: greeting
    if there_exists(["how are you", "how are you doing","how are you"+asis_obj.name]):
        engine_speak("I'm very well, thanks for asking " + person_obj.name)

    # 4: time
    if there_exists(["what's the time", "tell me the time", "what time is it","whats the time now"+asis_obj.name]):
    ...
    # open twitter
    if there_exists(["open twitter","twitter","go on twitter"]):
        search_term = voice_data.split("for")[-1]
